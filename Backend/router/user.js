...
//               await user.save();
//               const transport = nodemailer.createTransport({    // yaha pe changes krne pad.. sakhte hai
...
router.post("/create/user",
    ...
    async (req, res) => {
        try {
            ...
            res.status(200).json({ Status: "Pending", msg: "Please check your email", userId: user._id });
        } catch (error) {
            console.error("Error:", error);
...
//     if(!token || !_id){    // ismai chnages kr sakhte hai
...
router.post("/forgot/password", async (req, res) => {
    ...
    const existingToken = await ResetToken.findOne({ user: user._id });
    if (existingToken) {
        await existingToken.deleteOne();
    }

    const randomTxt = crypto.randomBytes(20).toString('hex');
    const hashedToken = await bcrypt.hash(randomTxt, 10);
    const resetToken = new ResetToken({
        user: user._id,
        token: hashedToken,
    });
...
router.put("/reset/password", async (req, res) => {
    ...
    const isMatch = await bcrypt.compare(token, resetToken.token); // Compare the plain token with the hashed token
    if (!isMatch) {
        return res.status(400).json("Token is not valid");
    }
